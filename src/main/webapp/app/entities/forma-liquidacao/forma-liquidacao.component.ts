...
export class FormaLiquidacaoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.formaLiquidacaoService
      ...
      .subscribe((res: HttpResponse<IFormaLiquidacao[]>) => this.paginateFormaLiquidacaos(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IFormaLiquidacao) {
    return item.id;
  }

  registerChangeInFormaLiquidacaos() {
    this.eventSubscriber = this.eventManager.subscribe('formaLiquidacaoListModification', () => this.loadAll());
  }

  delete(formaLiquidacao: IFormaLiquidacao) {
  ...
  protected paginateFormaLiquidacaos(data: IFormaLiquidacao[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.formaLiquidacaos = data;
  }
}
