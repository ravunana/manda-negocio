...
export class ImpostoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.impostoService
      ...
      .subscribe((res: HttpResponse<IImposto[]>) => this.paginateImpostos(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IImposto) {
    return item.id;
  }
  ...
  registerChangeInImpostos() {
    this.eventSubscriber = this.eventManager.subscribe('impostoListModification', () => this.loadAll());
  }

  delete(imposto: IImposto) {
  ...
  protected paginateImpostos(data: IImposto[], headers: HttpHeaders) {
